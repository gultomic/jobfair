    ...
    <body class="antialiased">
        <div class="relative min-h-screen bg-gray-100 bg-center sm:flex sm:justify-center sm:items-center bg-dots-darker selection:bg-red-500 selection:text-white">
            ...
            <div class="p-6 mx-auto max-w-7xl lg:p-8">
                ...
                <p class="text-3xl font-black text-center uppercase">{{ $item->jobfair->nama}}</p>

                <div class="mt-4 md:mt-16">
                    <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
                        <div class="mx-auto">
                            <span class="text-sm italic">Scan qrcode:</span>
                            {!! QrCode::size(200)->generate(route('event.qrcheckin', ['id' => $item->id])) !!}
                            <div class="my-4 text-center">
                                atau
                                <a
                                    class="px-4 py-1 text-sm text-white bg-blue-600 rounded"
                                    href="{{ route('event.checkin', ['id' => $item->id]) }}"
                                >Check In</a>
                                manual
                            </div>
                        </div>
